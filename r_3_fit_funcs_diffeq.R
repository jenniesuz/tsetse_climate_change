...
nll.pois <- function(parms=tsetse_params(),dat=count.times,temps.yearmon=b.temps$yearmon){ 
  ...
  if (parms$mu.a.k1 > 0.04 | parms$mu.a.k1 < 0.01 ) {         # penalise model fits where adult mortality at temperatures =< 25 is > 0.03
    ...
    simulate <- tsetse_mod(parms=parms)                           # run the model and save output to object 'simulate'
  
      data <- dat[!dat$counts %in% NA,]        # times have data for
      mean.tsetse <- round(data$counts,0)     # round observed counts
      ...
      est <- round(est$mean.sim[est$yearmon %in% data$yearmon],0)
