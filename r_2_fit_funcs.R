...
nll.pois <- function(parms=tsetse_params(),dat=temps.count){ 
  ...
  if (parms$mu.a.k1 > 0.04 | parms$mu.a.k1 < 0.01 ) {                                # penalise model fits where adult mortality at temperatures =< 25 is > 0.03
    ...
    simulate <- simPop(parms=parms)                           # run the model and save output to object 'simulate'
      ...
      est <- round(simulate[simulate$time %in% times,"A"],0)  # model output numbers of adult females at timepoints have data for
  
      subset <- simulate[124:363,"A"]                         # take simulated counts between ~ 1970 and 1990
