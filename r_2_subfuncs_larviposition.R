# Fits larviposition rate as a function of temperature
...
# and Hargrove (2004) Tsetse Population Dynamics. In: The Trypanosomiases. Eds Maudlin, I
# Holmes, P., Miles, M. Cabi Publishing.
# Data is for G. pallidipes

lrDat <- read.csv("data_larviposition.csv")               # load larviposition data
#****************Plot data**************************************
lrPlot <- ggplot(lrDat, aes(x=mean_temp,y=rate)) +
  xlim(low=15, high=33) +
  geom_point(size=1) +
...
d4=0.002
#**************Predicted mortality***********************************
lrPred <- lrFunc(lr.d1=d1
                 ,lr.d2=d2
                 ,temp=seq(15,34,0.1))
lrPred <- cbind.data.frame(temp = seq(15,34,0.1), lrPred = lrPred)
#******************Plot fit*****************************************
lrPlotFit <- lrPlot +
  geom_line(data=lrPred
            ,mapping=aes(x=temp,y=lrPred))
lrPlotFit
#********************************************************************